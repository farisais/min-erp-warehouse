...
class Po_model extends CI_Model
{
	...
	{
		$this->db->select('po.*, supplier.name AS supplier_name, mr.mr_number');
		$this->db->from('po');
        $this->db->join('supplier', 'po.supplier=supplier.id_supplier', 'INNER');
    ...
    {
        $this->db->select('po.*, supplier.name AS supplier_name');
		$this->db->from('po');
        $this->db->join('supplier', 'po.supplier=supplier.id_supplier', 'INNER');
    ...
    public function get_po_history($id_mr, $exclude_id = null)
    {
        ...
        $this->db->join('supplier as s', 's.id_supplier=po.supplier', 'INNER');
    ...
    {
        $this->db->select('po.*, supplier.name as supplier_name, supplier.address, mr.mr_number, mr.id_mr, mr.project_list,mr.date as mr_date, mr.status as mr_status');
        $this->db->from('po');
        $this->db->where('po.id_po', $id);
        $this->db->join('mr', 'mr.id_mr=po.mr', 'LEFT');
        $this->db->join('supplier', 'supplier.id_supplier=po.supplier', 'INNER');
        return $this->db->get()->result_array();
    }
