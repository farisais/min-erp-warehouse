...
function CreateData()
{
    load_content_ajax(GetCurrentController(), 172, null, null);
}
...
function EditData()
{
    ...
    if(row != null)
    {
        ...
        load_content_ajax(GetCurrentController(), 173 ,data_post, param);
    }
    else
    {
       ...
       if(confirm("Are you sure you want to delete app_config : " + row.name))
        {
            var data_post = {};
            data_post['id_config'] = row.id_config;
            load_content_ajax(GetCurrentController(), 174 ,data_post);
        }
    }
    else
    {
