...
$("#jqxExpander").jqxExpander({ width: '100%', expanded: false});
    ...
    $("#so-number").jqxInput({ source: dataAdapterSupplier, displayMember: "SO Number", valueMember: "id_so", height: 23});
    ...
    $('#select-so-grid').on('rowdoubleclick', function (event) 
    {
        ...
        var url_bom = "<?php echo base_url() ?>so/get_pl_product_from_so?id=" + data.id_so;
        $.get(url_bom, function(data){
            ...
            for(i=0;i<data['data'].length;i++)
            {
                //alert(data['data'][0]);
                var data_input = {};
                data_input['product_code'] = data['data'][i]['product_code'];
                data_input['id_product'] = data['data'][i]['id_product'];
                data_input['product'] = data['data'][i]['id_product'];
                data_input['product_name'] = data['data'][i]['product_name'];
                data_input['total_qty'] = data['data'][i]['total_qty'];
                data_input['unit']= data['data'][i]['unit'];
                data_input['unit_name'] = data['data'][i]['unit_name'];
                
                data_input['merk'] = data['data'][i]['merk'];
                data_input['merk_name'] = data['data'][i]['merk_name'];
    ...
    var url = "<?php if(isset($is_edit)){?><?php echo base_url()?>so/get_pl_product_from_so?id=<?php echo $data_edit[0]['so']; ?> <?php }?>";
    var source =
    {
        datatype: "json",
        datafields:
    ...
    $("#pl-product-grid").jqxGrid(
    {
        ...
        columns: [
            ...
            { text: 'Unit', dataField: 'unit', displayfield: 'unit_name', width: 100},
            { text: 'Quantity', dataField: 'total_qty',cellsformat: 'd2'},
            { text: 'Unit COGS', dataField: 'unit_cogs',cellsformat: 'd2'},
            { text: 'Total COGS', dataField: 'total_cogs',cellsformat: 'd2'}
        ]
    });
...
<div id='form-container' style="font-size: 13px; font-family: Arial, Helvetica, Tahoma">
    ...
    <div class="form-center" style="padding: 30px;">
        <div><h1 style="font-size: 18pt; font-weight: bold;">SO COGS</h1></div>
        <div>
