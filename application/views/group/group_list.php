...
function CreateData()
{
    load_content_ajax(GetCurrentController(), 167, null, null);
}
...
function EditData()
{
    ...
    if(row != null)
    {
        ...
        load_content_ajax(GetCurrentController(), 168 ,data_post, param);
    }
    else
    {
       ...
       if(confirm("Are you sure you want to delete group : " + row.name))
        {
            var data_post = {};
            data_post['id_group'] = row.id_group;
            load_content_ajax(GetCurrentController(), 169 ,data_post);
        }
    }
    else
    {
