...
class Po extends MY_Controller
{
    ...
    {
        $this->po_model->change_po_status($this->input->post('id_po'), 'void');
    }
    
    public function get_po_history()
    {
        echo "{\"data\" : " . json_encode($this->po_model->get_po_history($this->input->get('id_mr'), $this->input->get('id_po'))) . "}";
    }
    ...
    public function view_detail_po($id)
    {
         $data = array(
            "po_product" => $this->po_model->get_po_product_by_id($id),
            "data_edit" => $this->po_model->get_po_by_id($id),
