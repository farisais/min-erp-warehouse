...
class Report extends MY_Controller
{
    ...
    public function dn_report_template()
    {
        $this->load->model('dn_model');
        $dn = $this->dn_model->get_dn_by_id($this->input->get('id'));
        $dn_product = $this->dn_model->get_dn_product_by_id_dn($this->input->get('id'));
        $data = array();
        $data['company'] = $this->appsetting_model->get_app_config_by_name('company_name');
        $data['company_address'] = $this->appsetting_model->get_app_config_by_name('company_address');
        $data['customer_address'] = $dn[0]['address'];
        $data['customer_name'] = $dn[0]['customer_name'];
        $data['document_name'] = 'DELIVERY NOTES';
        $data['document_number'] = $dn[0]['no_dn'];
        $data['document_date'] = date('d/m/Y', strtotime($dn[0]['date']));
        $data['items'] = $dn_product;
        $this->load->view('templates/report/dn_template', $data);
    }
