...
class Mr extends MY_Controller
{
    ...
    {
        $dataMR = $this->mr_model->get_mr_by_id($id);
        if($dataMR[0]['status'] != 'close')
        {
        $this->mr_model->change_mr_status($this->input->post('id_mr'), 'void');
        }
    }
    ...
    public function view_mr_detail($id)
    {
        $data = array(
            'data_edit' => $this->mr_model->get_mr_by_id($id),
            'is_edit' => 'true',
    ...
    public function get_mr_product_open_po()
    {
        echo "{\"data\" : " . json_encode($this->mr_model->get_mr_product_open_po($this->input->get('id'))) . "}";
    }
}
?>
