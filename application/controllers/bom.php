...
class Bom extends MY_Controller
{
    ...
    public function get_bom_template_product_from_so()
    {
        echo "{\"data\" : " . json_encode($this->bom_model->get_bom_template_product_from_so($this->input->get('so'))) . "}";
    }
    ...
    {
        $this->bom_model->delete_bom($this->input->post('id_bom'));
